...
public class ActivityPads extends ListActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mAdapter = new SimpleCursorAdapter(mContext, R.layout.list_pad, null, new String[] {
                "name", "padid" }, new int[] { R.id.list_pad_name, R.id.list_pad_id });
    ...
    protected Dialog onCreateDialog(int id) {
        ...
        case DIALOG_ADD:
            ...
            // Custom view
            View layout = getLayoutInflater().inflate(R.layout.dialog_add_pad, null);
            builder.setView(layout);
            // Buttons actions
            final EditText name = (EditText) layout.findViewById(R.id.add_pad_name);
            final EditText padid = (EditText) layout.findViewById(R.id.add_pad_id);
                ...
                public void onShow(DialogInterface d) {
                    ...
                    button.setOnClickListener(new View.OnClickListener() {
                        public void onClick(View v) {
                            if (validateAdd(name, padid)) {
                                new AddPad().execute(name.getText().toString(), padid.getText()
                                        .toString(), mHostID + "");
                                dialog.cancel();
                            }
                        }
    ...
    private boolean validateAdd(EditText name, EditText padid) {
        boolean ok = true;
        if (name != null && name.getText().toString().length() == 0) {
            name.setError("You must Provide a pad name");
            ok = false;
        }
        if (padid != null && padid.getText().toString().length() == 0) {
            padid.setError("You must Provide a Pad ID");
            ok = false;
        }
        return ok;
    }
